# Диплом: сборка, тесты и отчёты

## Требования
- macOS, JDK 11
- Android SDK + эмулятор/устройство (minSdk 21)
...
# Инструментальные тесты (нужен девайс/эмулятор)
./gradlew :app:connectedDebugAndroidTest --no-daemon
```

## Allure из JUnit XML (CLI)
```bash
# 1) Загрузка и распаковка CLI выполняется автоматически по зависимостям
# 2) Генерация отчётов
./gradlew :app:generateAllureUnitReportFromJUnitXml \
          :app:generateAllureAndroidTestReportFromXml \
          :app:generateAllureAndroidTestAggReportFromXml --no-daemon

# 3) Просмотр через локальный сервер
./gradlew :app:openAllureUnitServer               # порт 5252
./gradlew :app:openAllureAndroidTestServer        # порт 5254
./gradlew :app:openAllureAndroidTestAggServer     # порт 5256
```

Статические пути отчётов:
- Unit: `reports/allure/unit`
- AndroidTest: `reports/allure/androidTest`
- AndroidTest (агрегат): `reports/allure/androidTest-agg`

Если Allure CLI недоступен, можно отдать отчёты как статику:
```bash
...
## APK в prod и дистрибутив
```bash
# Скопировать APK в app/prod/
./gradlew :app:copyDebugApkToProd --no-daemon

# Собрать архив dist/Foodium_diplom.zip (включит reports/ и APK)
./gradlew :app:buildDist --no-daemon
```

## Jacoco (если MR5 уже в main)
```bash
# unit
./gradlew :app:jacocoUnitTestReport --no-daemon
# androidTest (нужен девайс)
# ./gradlew :app:jacocoAndroidTestReport --no-daemon
```

Пути HTML-отчётов Jacoco (см. MR5):
- Unit: `app/build/reports/jacoco/jacocoUnitTestReport/html/index.html`
- AndroidTest: `app/build/reports/jacoco/jacocoAndroidTestReport/html/index.html`

Все отчёты собираются в `reports/` для удобной проверки.
...
- Генерация из JUnit XML (надёжный способ):
  ...
  - AndroidTest → HTML в `reports/allure/androidTest`:
    ```bash
    app/build/allure-cli/allure/allure-2.13.9/bin/allure generate app/build/outputs/androidTest-results/connected -c -o reports/allure/androidTest
    ```
...
- Запуск отчётов (локальный сервер):
  - Через локальный CLI в проекте:
    ...
    app/build/allure-cli/allure/allure-2.13.9/bin/allure open reports/allure/androidTest -p 5254
    ```
...
- Агрегированный отчёт androidTest (все androidTest вместе; рекомендуемый для защиты):
  - Генерация из всех JUnit XML:
    ```bash
    app/build/allure-cli/allure/allure-2.13.9/bin/allure generate app/build/outputs/androidTest-results/connected -c -o reports/allure/androidTest-agg
    ```
  - Открытие отчёта:
    ```bash
